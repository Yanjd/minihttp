...
bool Log::init(const char* filename,int log_buf_size,int max_lines,int max_queue_size)
{
    if(max_queue_size>=1)
    {
        m_is_async=true;
    ...
    if(p==NULL)
    {
        snprintf(log_full_name,256,"%d_%02d_%02d_%s",my_tm.tm_year+1900,my_tm.tm_mon+1,my_tm.tm_mday,filename);
    }
    else
    {
        strcpy(log_name,p+1);
